...
async def test_films_list(make_get_request, es_data_film, es_write_data):
    await es_write_data(es_data_film)
...
async def test_film_not_exist(make_get_request):
    ...
    assert response['status'] == 404  # Я пока фастапи в этой ветке не менял, поэтому 404, а не 200
